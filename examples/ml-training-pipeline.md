...
| `environment-lock` | Pinned CUDA + framework + tokenizer versions in a lockfile; GPU driver and arch recorded in each run's meta |
| `data-fingerprint` | SHA-256 of the train/eval shards and the tokenizer vocab in `meta.json`; "did the data change?" is answered from the hash, not from memory |
...
| `reproducibility-provenance` | Base seed logged; deterministic dataloader ordering; the results figure is regenerated by one command from the logged metrics |
...
| `external-positioning` | Benchmark claims stated against the published SOTA number with a train/test contamination check done *before* claiming; ablations separate the architecture's effect from the recipe's |
